...
import com.example.demo.dtos.LatestSplitDto;
...
public class TransactionService {
	...
	private double calculatesplit(int n,double amount) {
		...
		return Math.round((amount/n)*100.0)/100.0;
...
;				          }
	...
	public LatestSplitDto getLatestSplit(HttpSession session) {
		
		UserEntity user= (UserEntity)session.getAttribute("LoggedInUser");
		
		LatestSplitDto latestSplit = new LatestSplitDto();
		
		TransactionEntity latest= transactionRepository.findLatestTransaction(user.getId());
		Object shareMembersRaw = transactionShareRepository.getShareMembersRaw(latest.getId());
		
		
		
		List<String> shareMembersList = List.of();
		double shareAmount = 0;
		if(shareMembersRaw != null ) {
			  Object[] row = (Object[])shareMembersRaw;
			  
			  shareMembersList = List.of(((String) row[0]).split(","));
			   shareAmount = ((Number) row[1]).doubleValue();
		}
		
		
		latestSplit.setTransactionName(latest.getTransactionName())
		.setAmount(latest.getAmount()).setDateTime(latest.getDateTime()).setId(latest.getId()).setShareAmount(shareAmount)
		.setShareMembers(shareMembersList);
		return latestSplit;
	}
